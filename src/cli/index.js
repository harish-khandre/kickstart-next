...
export default async function runCli() {
  ...
  const promptText = async (message, placeholder = "my-app") => {
  ...
  const promptConfirm = async (message) => handleCancel(await confirm({ message }))
  const promptSelect = async (message, options) => handleCancel(await select({ message, options }))
  ...
  if (auth !== "none") {
    const authProviders = await handleCancel(
      await multiselect({
